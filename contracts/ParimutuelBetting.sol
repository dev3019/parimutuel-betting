...
contract ParimutuelBetting {
    ...
    event PayoutFailed(address indexed user, uint256 amount);
    ...
    function _distributeWinnings(
        ...
        // Calculate total pool from all bets placed on all options
        uint256 totalPool = 0;
        for (uint256 i = 0; i < prediction.options.length; i++) {
            totalPool += prediction.totalBets[prediction.options[i]];
        }
        require(totalPool > 0, "No bets placed on prediction");

        uint256 totalCorrectBets = prediction.totalBets[_winner];
        require(totalCorrectBets > 0, "No bets placed on winning option");
        ...
        for (uint256 i = 0; i < prediction.participants.length; i++) {
            ...
            if (userBet > 0) {
                uint256 payout = (userBet * totalPool) / totalCorrectBets;
                bool success = payTo(participant, payout);
                if (success) {
                    emit Payout(participant, payout);
                } else {
                    emit PayoutFailed(participant, payout);
                }
            }
        }
    }
    ...
    function getUserBet(uint256 _predictionId, address _user, string memory _option) external view returns (uint256) {
        return predictions[_predictionId].bets[_user][_option];
    }

    function getTotalBets(uint256 _predictionId, string memory _option) external view returns (uint256) {
        return predictions[_predictionId].totalBets[_option];
    }
